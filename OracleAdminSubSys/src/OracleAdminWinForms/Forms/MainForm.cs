...
public class MainForm : Form
{
    ...
    private readonly TextBox _txtUserSearch = UiHelper.CreateTextBox("Tìm username");
    private readonly TextBox _txtRoleSearch = UiHelper.CreateTextBox("Tìm role");
    private readonly TextBox _txtObjectOwnerFilter = UiHelper.CreateTextBox("Owner filter");

    private readonly ComboBox _cboGrantPrincipalType = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantPrincipal = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantMode = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantSystemPriv = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantRoleName = UiHelper.CreateComboBox();
    private readonly TextBox _txtGrantOwner = UiHelper.CreateTextBox("LAB_OWNER");
    private readonly ComboBox _cboGrantObjectName = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantObjectType = UiHelper.CreateComboBox();
    private readonly ComboBox _cboGrantObjectPriv = UiHelper.CreateComboBox();
    ...
    private readonly ComboBox _cboRevokePrincipalType = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokePrincipal = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokeMode = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokeSystemPriv = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokeRoleName = UiHelper.CreateComboBox();
    private readonly TextBox _txtRevokeOwner = UiHelper.CreateTextBox("LAB_OWNER");
    private readonly ComboBox _cboRevokeObjectName = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokeObjectType = UiHelper.CreateComboBox();
    private readonly ComboBox _cboRevokeObjectPriv = UiHelper.CreateComboBox();
    private readonly CheckedListBox _clbRevokeColumns = new() { Height = 140, CheckOnClick = true };

    private readonly ComboBox _cboExplorerPrincipalType = UiHelper.CreateComboBox();
    private readonly ComboBox _cboExplorerPrincipal = UiHelper.CreateComboBox();
    ...
    public MainForm(DbConnectionSettings settings)
    {
        _settings = settings;
    ...
    private TabPage BuildObjectsTab()
    {
        ...
        // Mặc định chỉ hiển thị object demo trong LAB_OWNER.
    ...
    private void LoadAll()
    {
        ...
        {
            UiHelper.ShowError(ex, "Khởi động thất bại");
        }
    }
    ...
    private void CreateUser()
    {
        ...
        try
        {
            ...
            UiHelper.ShowInfo("Tạo user thành công.");
            RefreshLists();
        }
        catch (Exception ex)
        {
    ...
    private void AlterUserPassword()
    {
        ...
        {
            UiHelper.ShowInfo("Hãy chọn một user trước.");
            return;
        }
     ...
     private void AlterUserDefaultTablespace()
    {
        ...
        try
        {
            ...
            LoadUsers();
            UiHelper.ShowInfo("Đổi Default Tablespace thành công.");
        }
        catch (Exception ex)
        {
            UiHelper.ShowError(ex);
    ...
    private void AlterUserTemporaryTablespace()
    {
        ...
        try
        {
            ...
            LoadUsers();
            UiHelper.ShowInfo("Đổi Temporary Tablespace thành công.");
        }
        catch (Exception ex)
        {
            UiHelper.ShowError(ex);
    ...
    private void AlterUserProfile()
    {
        ...
        try
        {
            ...
            LoadUsers();
            UiHelper.ShowInfo("Đổi Profile thành công.");
        }
        catch (Exception ex)
        {
            UiHelper.ShowError(ex);
    ...
    private void ToggleSelectedUserLock()
    {
        ...
        try
        {
            ...
            UiHelper.ShowInfo(shouldLock ? "Đã khóa user." : "Đã mở khóa user.");
            LoadUsers();
        }
        catch (Exception ex)
        {
    ...
    private void DropSelectedUser()
    {
        ...
        if (!UiHelper.Confirm($"Bạn có chắc muốn DROP USER {username} CASCADE không?"))
            return;

        try
        {
            _service.DropUser(username, cascade: true);
            UiHelper.ShowInfo("Đã drop user.");
            RefreshLists();
        }
        catch (Exception ex)
        {
    ...
    private void CreateRole()
    {
        ...
        try
        {
            ...
            UiHelper.ShowInfo("Tạo role thành công.");
            RefreshLists();
        }
        catch (Exception ex)
        {
    ...
    private void AlterRolePassword()
    {
        ...
        {
            UiHelper.ShowInfo("Hãy chọn một role trước.");
            return;
        }
        ...
        try
        {
            ...
            UiHelper.ShowInfo("Cập nhật role thành công.");
            LoadRoles();
        }
        catch (Exception ex)
        {
    ...
    private void DropSelectedRole()
    {
        ...
        if (!UiHelper.Confirm($"Bạn có chắc muốn DROP ROLE {roleName} không?"))
            return;

        try
        {
            _service.DropRole(roleName);
            UiHelper.ShowInfo("Đã drop role.");
            RefreshLists();
        }
        catch (Exception ex)
        {
    ...
    private void ExecuteGrant()
    {
        try
        {
            ...
            {
                UiHelper.ShowInfo("GRANT thành công.");
            }
    ...
    private void ExecuteRevoke()
    {
        try
        {
            ...
            UiHelper.ShowInfo("REVOKE thành công.");
            LoadPrivilegeExplorer();
        }
        catch (Exception ex)
        {
    ...
    private void LoadPrivilegeExplorer()
    {
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(principal))
            {
                UiHelper.ShowInfo("Hãy chọn principal để xem quyền.");
                return;
            }
